...
const AssignCombo = ({ id, tt }: any) => {

    const [open, setOpen] = useState(false);
    const [value, setValue] = useState(tt);
    ...
    //         { merge: true }
    //     ).then(async () => {
    //         const ref = query(collection(db, "reviewers"), where("reviewerName", "==", value));
    //         await getDocs(ref).then((doc) => {
    //             console.log(doc.docs);
    //             // if (doc.docs[0].data().assignedPapers.includes(id)) {
    //             //     updateDoc(reviewerRef, {
    //             //         assignedPapers: arrayRemove(id),
    //             //     });
    //             // }
    //         });
    //         // await getDoc(reviewerRef).then((doc) => {
    //         //     console.log(doc.data()!.assignedPapers.includes(id));
    //         // });
